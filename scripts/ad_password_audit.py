...
def main() -> int:
    ...
    csv_path = build_report_path(out_dir, "password_audit", ".csv")
    xlsx_path = build_report_path(out_dir, "password_audit", ".xlsx")
    html_path = build_report_path(out_dir, "password_audit", ".html")

    export_csv(result.dataframe, csv_path)
    export_xlsx(result.dataframe, xlsx_path, result.summary)
    export_html(result.dataframe, html_path, "Отчет по аудиту паролей", result.summary, result.notes)
